...
Table::Table(const std::string &db_name) {
    ...
    this->curr_db = std::fstream(DATA_PATH + db_name + ".db", std::ios::in | std::ios::out | std::ios::app);
    this->epoch_db = std::fstream(DATA_PATH + db_name + "_epochs.db", std::ios::in | std::ios::out | std::ios::app);
...
// Searches for the epoch in the cache and if it is not found, then it will search the file
RecordData Table::search(uint64_t epoch) {
    ...
    if (this->epochs.exists(epoch)) { return this->epochs.get(epoch); }
...
void Table::save() {
    // Writes the order book to the file
    this->curr_db.close();
    this->curr_db.open(DATA_PATH + this->db_name + ".db", std::ios::out | std::ios::trunc);
...
void Table::update(uint64_t epoch, uint64_t last_epoch, float price, unsigned int quantity, Side side, bool add) {
    // Update order book
    if (epoch < last_epoch) {
        throw std::runtime_error("New epoch is less than last added epoch which cannot happen");
    }

    // To be updated (will only work if the last added epoch is still in the cache)
    ...
    if (add) this->order_book.add(price, quantity, side);
    else this->order_book.remove(price, quantity, side);
    ...
    if (this->epochs.size() == 0) { this->epochs.put(epoch, data); return; }

    // Check to see if the key is in the cache and perform the appropriate action
    if (this->epochs.exists(epoch)) { this->epochs.update(epoch, data); }
    else { this->epochs.put(epoch, data); }
}
