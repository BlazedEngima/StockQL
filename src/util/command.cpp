...
const static WordMap keywords = {
    {"INSERT", STATEMENT_ADD},    {"insert", STATEMENT_ADD},
    {"ADD", STATEMENT_ADD},       {"add", STATEMENT_ADD},
    {"NEW", STATEMENT_ADD},       {"new", STATEMENT_ADD},
    {"TRADE", STATEMENT_TRADE},   {"trade", STATEMENT_TRADE},
    {"DELETE", STATEMENT_REMOVE}, {"delete", STATEMENT_REMOVE},
    {"CANCEL", STATEMENT_REMOVE}, {"cancel", STATEMENT_REMOVE},
    {"SELECT", STATEMENT_QUERY},  {"select", STATEMENT_QUERY},
    {"EXIT", STATEMENT_EXIT},     {"exit", STATEMENT_EXIT},
    {"q", STATEMENT_EXIT},        {"Q", STATEMENT_EXIT},
    {"quit", STATEMENT_EXIT},     {"QUIT", STATEMENT_EXIT}};
...
bool isNumber(const std::string &in) {
  return !in.empty() && std::find_if(in.begin(), in.end(), [](unsigned char c) {
...
void Command::parse() {
  ...
  if (isNumber(this->command_args[0])) {
    ...
    // If it is a log, then treat it as an add command and insert it into the
    // order book
    this->epoch = std::stoull(this->command_args[0]);
    this->symbol = this->command_args[2];
    this->side =
        (this->command_args[3] == "BUY" || this->command_args[3] == "buy")
            ? BUY
            : SELL;
    this->type = (keywords.find(this->command_args[4]) == keywords.end())
                     ? STATEMENT_COMMAND_UNRECOGNIZED
                     : keywords.at(this->command_args[4]);
  ...
  switch (this->type) {
    case STATEMENT_QUERY: {
      ...
      if (this->command_args.size() == 3) {
        ...
        // Enables query searches such as "select symbol *" and "select symbol
        // top" Improvements can be made here
...
std::istream &operator>>(std::istream &istream, Command &command) {
  ...
  command.command_args = StringVector(std::istream_iterator<std::string>{iss},
...
MetaCommandResult Command::executeStatement(std::ostream &os) {
  ...
  switch (this->type) {
    ...
    case STATEMENT_ADD: {
      this->table->update(this->epoch, this->last_epoch, this->price,
                          this->quantity, this->side, true);
