...
unsigned int LRUCache::size() const { return this->cache_map.size(); }// Get size of LRUCache

// Helper function to clean the cache
void LRUCache::clean() {
    while (this->cache_map.size() > this->capacity) {
        // Get the key of the last element
        auto last = this->cache_list.end(); last--;
...
bool LRUCache::exists(const uint64_t &key) {
    ...
    if (this->cache_list.front().first == key) { return true; }
